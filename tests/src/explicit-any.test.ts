...
import explicitAnyPlugin from '../../src/ts-migrate-plugins/plugins/explicit-any';
...
`);
  ...
  it.only('adds an import for an added type', async () => {
    const text = `
    export const uploadTempPhoto2 =  async(
      sectionKey: string,
      file: File,
      index = 0
    ): AppThunk<Promise<{ url: null }>> => {
      return (dispatch) => {
        return { url: null };
      };
    };`;
