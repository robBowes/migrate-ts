...
import reactPropsPlugin from '../../src/ts-migrate-plugins/plugins/react-props';
...
const Foo = forwardRef<any, FooProps>(({ foo }, ref) => <div ref={ref}>{foo}</div>)
...
const Foo = React.forwardRef<any, FooProps>(({ foo }, ref) => <div ref={ref}>{foo}</div>)
...
const Foo = ({ foo }: FooProps) => <div>{foo}</div>;
...
const Foo = ({ foo }: FooProps, { bar }) => <div>{foo}</div>;
Foo.contextTypes = contextTypes;
...
const Foo = ({ children, className }: FooProps) => {
  return <div className={className}>children</div>;
};
...
import Link from ':dls-legacy-16/components/deprecated/Link_20190703';
import Spacing from ':dls-legacy-16/components/exp/Spacing';
type PaymentPlanOptionExplanationProps = {
    subtitle: string;
    learnMoreLinkText: string;
    paymentPlanModalContent: any; // TODO: Types.shape()
    showPaymentPlanModal: (...args: any[]) => any;
};
export default class PaymentPlanOptionExplanation extends React.PureComponent<PaymentPlanOptionExplanationProps> {
  render() {
    const {
      subtitle,
      learnMoreLinkText,
