        ...
        if (
          ...
          if (
            ...
            ts.isObjectLiteralExpression(
              variableDeclaration.initializer.arguments[0]
            ) &&
            ...
            if (
              !typesAndInterfaces.find(
                (tNode) =>
                  tNode.name.text === variableDeclaration.name.getText()
              )
            ) {
              updates.push({
                kind: "insert",
            ...
            const updatedVariableDeclaration =
          ...
          if (
            ...
            insertPropTypesRequireableNode();
            const shapeNode = variableDeclaration.initializer
              .arguments[0] as ts.CallExpression;
            const shapeName = variableDeclaration.name.getText();
...
function getTypeForTheShape(
  ...
  const propsTypeAlias = ts.factory.createTypeAliasDeclaration(
    ...
    undefined,
    isArrayShapeType
      ? ts.factory.createArrayTypeNode(shapeTypeVariable)
      : shapeTypeVariable
  );
